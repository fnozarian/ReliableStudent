...
def eval_class(gt_annos,
    ...
    recall = np.zeros(
        [num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
    detailed_stats = np.zeros(
        [num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS, 5])  # TP, FP, FN, Similarity, thresholds
...
def do_eval(gt_annos,
    ...
    if PR_detail_dict is not None:
        ...
        stats_3d = {'precision': ret['raw_precision'], 'recall': ret['raw_recall'],
                    'detailed_stats': ret['detailed_stats']}
        PR_detail_dict['3d_'] = stats_3d
